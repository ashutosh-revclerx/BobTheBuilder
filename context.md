...
- **Python LLM microservice** at `services/llm/` (FastAPI + Gemini 2.5 Flash + pydantic post-validation + 1-shot repair retry + programmatic palette-swap variant generator). System prompt v2 with worked example. In `docker-compose.yml` with hot-reload.
...
**See [`roadmap.md`](./roadmap.md) for what's next** — Sprint 3 (inline validation, "Regenerate" button, JSON editor pane, draft placeholder). Phase 3.11–3.16, Phase 4, Phase 5 stay deferred. **See [`example.md`](./example.md) for the platform how-to + binding rules cheatsheet.**
