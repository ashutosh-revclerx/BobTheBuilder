...
from .schemas import DashboardConfig, GenerateRequest, GenerateResponse, GeneratedVariant, ResourceContext

logger = logging.getLogger("llm.variants")
...
async def generate_variants(
    *,
    prompt: str,
    resources: list[dict[str, Any]] | list[ResourceContext] | None = None,
    docs_urls: list[str] | None = None,
    variant_count: int = 4,
) -> GenerateResponse:
    """Generate a base dashboard, then derive visual/layout variants.

    This is the in-process replacement for the old standalone LLM module's
    ...
    from .openai_client import generate_dashboard_config as generate_dashboard_config_openai
    ...
    base_config: DashboardConfig | None = None
    ...
    enriched = enrich_config(base_config, req.prompt)
    return GenerateResponse(
        success=True,
        configs=derive_variants(
            enriched,
            "Dashboard",
            req.variant_count,
