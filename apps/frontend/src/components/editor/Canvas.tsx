...
export default function Canvas() {
  ...
  // Execute onLoad queries once on initial component list arrival
  useEffect(() => {
  ...
  const tables = components.filter((c) => c.type === 'Table' || c.type === 'LogsViewer');
  const controls = components.filter((c) => c.type === 'Button');
  ...
  components.forEach((comp) => {
    ...
    } else if (comp.type === 'Table' || comp.type === 'LogsViewer') {
      if (!groupOrder.find((g) => g.type === 'table')) {
        groupOrder.push({ type: 'table', items: tables });
        tables.forEach((c) => seen.add(c.id));
      }
    } else if (comp.type === 'Button') {
      if (!groupOrder.find((g) => g.type === 'control')) {
        groupOrder.push({ type: 'control', items: controls });
        controls.forEach((c) => seen.add(c.id));
      }
    }
  });
