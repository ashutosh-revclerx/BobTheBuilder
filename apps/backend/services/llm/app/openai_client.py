"""
OpenAI fallback for the LLM service.
...
def _format_sdk_error(exc: Exception) -> str:
    ...
    if "invalid_api_key" in text or "Incorrect API key" in text:
        return "OpenAI API key is invalid. Check OPENAI_API_KEY in the LLM service env."
...
def _client() -> Any:
    ...
    if not api_key:
        raise OpenAIError("OPENAI_API_KEY is not set in the LLM service env")
    try:
...
def generate_dashboard_config(system_prompt: str, user_prompt: str) -> DashboardConfig:
    ...
    try:
        ...
        raw = _call_openai(system_prompt, repair_prompt)
        try:
            return DashboardConfig.model_validate(json.loads(raw))
