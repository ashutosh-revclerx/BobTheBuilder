...
export function useTruncatedText(
  ...
  const cacheRef = useRef<Map<string, { display: string; isTruncated: boolean }>>(new Map());
  ...
  try {
    ...
    return { display: text.slice(0, maxChars - 1) + '…', isTruncated: true };
  }
}
...
export function useKineticWidth(value: string): number {
  ...
  useEffect(() => {
    if (!value) { setWidth(80); return; }
    try {
      const prepared = prepareWithSegments(value, NAME_FONT);
      const measured = measureNaturalWidth(prepared);
      setWidth(Math.ceil(measured) + 32); // 16px padding each side
    } catch {
      setWidth(value.length * 9 + 32);
    }
  }, [value]);
